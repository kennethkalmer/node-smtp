...
SMTPSession.prototype.receiver = function() {
  ...
  if( this.listeners('recipient').length <= 0 ) {
    session.sendOk();
  } else {
    var promise = new process.Promise();
    promise.addCallback( function() {
    ...
    this.emit( 'recipient', this.recipientAddress, promise );
  }
}
...
SMTPSession.prototype.sender = function() {
  ...
  if ( this.listeners('sender').length <= 0 ) {
    session.ok();
  } else {
    var promise = new process.Promise();
    promise.addCallback( function() {
    ...
    this.emit( 'sender', this.fromAddress, promise );
  }
}
...
SMTPSession.prototype.dataReceived = function(){
  ...
  if( re.test( this.buffer ) ) {
    ...
    if( this.listeners('data').length <= 0 ) {
      ...
      this.emit( 'data', session.data, promise );
    }
  }
}
